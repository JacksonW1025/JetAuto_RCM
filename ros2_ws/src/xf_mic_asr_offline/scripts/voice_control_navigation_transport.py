    ...
    def words_callback(self, msg):
        ...
        if words is not None and words not in ['唤醒成功(wake-up-success)', '休眠(Sleep)', '失败5次(Fail-5-times)',
            ...
            if words == '导航搬运' or words == 'navigate and transport':
                ...
                msg.data.x = float(self.pick_position[0])
                msg.data.y = float(self.pick_position[1])
                msg.data.roll = float(self.pick_position[2])
                msg.data.pitch = float(self.pick_position[3])
                msg.data.yaw = float(self.pick_position[4])
